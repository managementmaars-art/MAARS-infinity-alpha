...
SYSTEMS = [
    ...
    ("Verification Civilization", "Multi-verifier fact-checking with 6-dimensional consensus: completeness, correctness, source grounding, actionability, compliance, professionalism. Default verifier pool (fact, source, hallucination) runs each output in parallel and votes on pass/fail. Unanimous flag + weighted consensus score.", "POST /api/infinity/verify, GET /api/infinity/verify/history"),
    ("Trust Analytics", "0-100 trust score per agent, computed from success rate, latency profile, output quality, and consistency over trailing verification history. Leaderboard and low-trust alerts; scores persist in trust_scores collection.", "GET /api/infinity/governance/trust/{entity_type}/{entity_id}, GET /api/infinity/governance/trust/leaderboard"),
    ("Memory Hierarchy (4-Tier)", "Working memory (task-scoped), episodic memory (event sequences + lessons), semantic memory (concepts, cross-referenced), knowledge graph (entity-relationship with provenance). Integrated via agent_runtime so every task reads from and writes to all four tiers.", "GET/POST /api/infinity/memory/working|episodic|semantic|graph/*"),
    ("Knowledge Graph", "Entity-relationship graph with typed nodes, typed edges, traversal, path-finding, search, and stats. Feeds semantic memory, powers cross-agent reasoning, and captures provenance for every relationship added.", "POST /api/infinity/memory/graph/entity, /relationship, GET /traverse, /search, /paths, /stats"),
    ("Task Graph Execution Kernel", "DAG-based task execution with checkpoints, parallel branches, resumability, failure isolation per node, and re-entrant scheduling. Owns kernel-level policy enforcement and budget gates.", "POST/GET /api/kernel/graphs, /api/kernel/graphs/{id}/execute, /api/kernel/graphs/{id}/state"),
    ("Autonomy Tier System", "10-level autonomy scale from T0 (advisory only) to T9 (unsupervised). Every action is gated by its agent's tier, environment, and policy set. Tier escalation requires approval; tier downgrade happens automatically on verification fail or incident.", "Enforced in orchestrator/commander.py, kernel scheduler, and approvals route"),
    ("Budget Controller", "Per-user, per-agent, per-environment credit caps with soft (80%) and hard (100%) thresholds. Budget enforcement agent trips circuit breakers on overspend; spend analytics roll up to admin dashboard.", "GET /api/admin/budget, PUT /api/admin/budget/limits"),
    ("Circuit Breakers", "Auto-trip on spend spike, error rate, retry storm, or policy violation. Half-open probe after cooldown; full reset requires operator or passing incident review. Per-provider and per-agent scope.", "Governed by governance/policy.py and monitored by observability_incident network"),
    ("Integration Hub", "28 external integrations with unified config schema, OAuth flows, credential vaulting, runtime-support classification (live/hybrid/planned), and capability declarations per integration.", "GET /api/enterprise/integrations, POST /api/enterprise/integrations/{id}/connect"),
    ("Universal AI Gateway", "OpenAI-compatible gateway at /v1/chat/completions exposing 33 providers and 175,609+ models (609 curated + HuggingFace pass-through). Accepts maars-sk-* keys, enforces budget + rate-limit, emits webhook events.", "POST /api/v1/chat/completions, GET /api/v1/models, POST /api/v1/models/compare"),
    ("Skill Library & RAG", "58,422-document skill corpus auto-matched to every agent at creation time and ingested into the knowledge_chunks RAG index. Shared across 30+ AI-tool integration surfaces via canonical .agents/skills/ source.", "Ingestion: backend/scripts/ingest_skills.py; RAG retrieval in routes/chats.py"),
    ("Policy Engine", "Central governance rules: autonomy caps, action gating, jurisdictional compliance, content moderation, data residency. Rules apply per environment; violations logged and trigger remediation.", "GET/POST /api/admin/policies, enforced in every kernel call"),
    ("Audit Log", "Immutable record of every action (who/what/when/why/outcome). 7-year retention in production, queryable + exportable. Tamper-evident via hash chain.", "GET /api/admin/audit-log, POST /api/admin/audit-log/export"),
    ("Incident Ledger & Recovery", "Every fault captures timeline, severity, blast radius, remediation, and RCA. Quarantine → rollback → retry flow with automatic containment; incidents roll up to the Recovery & Resilience network agents.", "GET /api/enterprise/incidents, POST /api/enterprise/incidents/{id}/resolve"),
    ("Approvals (Human-in-Loop)", "Approval queue for gated actions — approve, reject, edit, or escalate. Autonomy tier and risk profile determine which actions route to approvals; every decision is audited.", "GET/POST /api/approvals, PUT /api/approvals/{id}/decision"),
    ("Environment Segregation", "Four isolated environments (Simulation, Sandbox, Staging, Production) with independent policies, budgets, integrations, and data stores. Cross-environment promotion gated by verification + approval.", "PUT /api/enterprise/system/environment, GET /api/admin/environments"),
    ("Social Media Command", "10-platform social orchestration (Facebook, Instagram, Twitter/X, TikTok, WhatsApp, Viber, LINE, LinkedIn, YouTube, Telegram) with post, boost, DM, cold-email, cold-call, and translate actions. Geographic and demographic ad targeting.", "GET/POST /api/social/* across 10 platform endpoints"),
    ("Campaign Builder", "Multi-channel campaign planning with AI-generated variants and A/B testing matrix. Produces cross-platform creative packs, scheduling, and attribution-ready tracking.", "POST /api/campaigns, GET /api/campaigns/{id}/variants"),
    ("Team Builder", "Compose any subset of the 458-agent roster into a named team with a shared mission, shared memory scope, shared budget, and cross-agent handoff rules. Teams appear as first-class collaborators in the orchestration graph.", "POST/GET/PUT /api/agent-teams, POST /api/agent-teams/{id}/dispatch"),
]

"""OpenAI provider adapter — delegates execution to the existing Universal Gateway call path."""
...
class OpenAIProvider(BaseProvider):
    ...
    def __init__(self, *, api_key: str = "", base_url: Optional[str] = None, timeout: float = 30.0):
        super().__init__(api_key=api_key, base_url=base_url or "https://api.openai.com", timeout=timeout)

    async def execute_chat_completion(self, *, model, messages, params=None) -> dict[str, Any]:
        # Delegate to the existing low-level caller so we don't fork two code paths.
        ...
        for m in messages:
            if m.get("role") == "system":
                ...
                tool_msgs.append(m)
        text = await call_direct_llm("openai", model, sys_prompt.strip(), user_content, tool_msgs, self.api_key)
        return {
    ...
    async def estimate_usage(self, *, model, messages, max_output_tokens=512) -> UsageEstimate:
        ...
        pricing = MODEL_COSTS_MAP.get(model, {"input": 2.5, "output": 10.0})

"""Anthropic provider adapter — delegates to existing call path, ships a distinct health check."""
...
class AnthropicProvider(BaseProvider):
    ...
    def __init__(self, *, api_key: str = "", base_url: Optional[str] = None, timeout: float = 30.0):
        super().__init__(api_key=api_key, base_url=base_url or "https://api.anthropic.com", timeout=timeout)
    ...
    async def execute_chat_completion(self, *, model, messages, params=None) -> dict[str, Any]:
        ...
        for m in messages:
            if m.get("role") == "system":
                ...
                tool_msgs.append(m)
        text = await call_direct_llm("anthropic", model, sys_prompt.strip(), user_content, tool_msgs, self.api_key)
        return {
    ...
    async def estimate_usage(self, *, model, messages, max_output_tokens=512) -> UsageEstimate:
        ...
        pricing = MODEL_COSTS_MAP.get(model, {"input": 3.0, "output": 15.0})

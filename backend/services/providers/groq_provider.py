"""Groq provider adapter — OpenAI-compatible, optimized for fast inference."""
...
class GroqProvider(BaseProvider):
    ...
    def __init__(self, *, api_key: str = "", base_url: Optional[str] = None, timeout: float = 30.0):
        super().__init__(api_key=api_key, base_url=base_url or "https://api.groq.com/openai", timeout=timeout)
    ...
    async def execute_chat_completion(self, *, model, messages, params=None) -> dict[str, Any]:
        ...
        for m in messages:
            if m.get("role") == "system":
                ...
                tool_msgs.append(m)
        text = await call_direct_llm("groq", model, sys_prompt.strip(), user_content, tool_msgs, self.api_key)
        return {
    ...
    async def estimate_usage(self, *, model, messages, max_output_tokens=512) -> UsageEstimate:
        # Groq is priced very low — default to cheap if model is unknown.
        from services.llm_service import MODEL_COSTS_MAP
        pricing = MODEL_COSTS_MAP.get(model, {"input": 0.05, "output": 0.08})

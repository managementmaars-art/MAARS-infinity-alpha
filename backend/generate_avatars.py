...
Routes through the MAARS media router (Gemini/OpenAI image chain).
...
async def generate_single_avatar(agent, idx, total, db):
    """Generate a single avatar through the media router and save it."""
    ...
    async with semaphore:
        try:
            ...
            image_bytes, meta = await route_image(prompt=prompt, quality="standard")
            ...
            if image_bytes:
                ...
                logger.info(f"[{idx}/{total}] OK {name} -> {file_path.name} ({len(image_bytes)} bytes) via {meta.get('provider')}/{meta.get('model')}")
                return True
            else:
...
async def main():
    ...
    for batch_start in range(0, total, batch_size):
        ...
        for i, agent in enumerate(batch):
            ...
            tasks.append(generate_single_avatar(agent, idx, len(agents), db))

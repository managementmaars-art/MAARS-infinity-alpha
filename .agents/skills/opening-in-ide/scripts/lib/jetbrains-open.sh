...
find_nearest_webstorm_context_dir() {
  local dir="$1"
  local marker
  local markers=(.idea package.json pnpm-workspace.yaml yarn.lock package-lock.json bun.lock bun.lockb tsconfig.json jsconfig.json)
...
main() {
  ...
  if [[ "$context_mode" == "rider" ]]; then
    if solution="$(find_nearest_project_file "$search_dir" "sln")"; then
      open_with_context "$solution"
    elif project="$(find_nearest_project_file "$search_dir" "csproj")"; then
      open_with_context "$project"
    fi
  elif [[ "$context_mode" == "webstorm" ]]; then
    if context_dir="$(find_nearest_webstorm_context_dir "$search_dir")"; then
      open_with_context "$context_dir"
    fi
  fi

  if [[ "$open_mode" == "file" && -n "$line" ]]; then
    launch_ide --line "$line" "$target"
    exit 0
  fi

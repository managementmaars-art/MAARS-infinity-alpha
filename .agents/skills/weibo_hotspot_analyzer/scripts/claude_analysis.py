...
请基于以下微博热搜话题，生成 3 个**与众不同、有独特创意**的产品创意，并简要说明这个热搜的内容。
...
## ⚠️ 严格禁止（违反直接扣50分）
- **禁止**使用"内容创作助手"、"知识卡片"、"热点素材库"、"创作灵感生成"这种**通用模板名称**
- **禁止**3个创意使用相同的产品形态（如都是工具类、都是知识卡片）
- **禁止**核心功能列表完全相同
...
**每个创意需包含**：
1. **产品名称**: 简洁易记，体现热点元素 **(必须2-8个字，拒绝"xxx助手"、"xxx工具")**
2. **综合评分**: 0-100分 (有趣度×0.8 + 有用度×0.2)
3. **有趣度评分**: 0-100分
4. **有用度评分**: 0-100分
5. **核心功能**: 3-5个**具体、有画面感**的功能点（禁止"热点素材库"、"创作灵感生成"这种泛泛而谈）
6. **目标用户**: 用户画像描述 (年龄、兴趣、需求场景)
7. **产品描述**: 100字以内，**必须有画面感、场景感**
...
{{
  ...
  "ideas": [
    {{
      ...
      "fun_score": 92,
      "use_score": 45,
      "features": ["真实通告模拟：在多个剧组间极限转场", "危机公关系统：应对替身穿帮危机", "剧本记忆挑战：同时记忆古装现代台词"],
      "target_users": "18-30岁泛娱乐用户、追星族、模拟经营游戏爱好者",
      "description": "一款硬核时间管理手游，扮演爆红演员体验三个剧组同时开工的极限操作"
    }},
    {{
      "name": "脑洞串戏编辑器",
      "score": 78,
      "fun_score": 85,
      "use_score": 60,
      "features": ["台词乱入生成器", "跨剧cp混剪", "经典台词替换"],
      "target_users": "短视频创作者、追剧粉丝",
      "description": "把不同剧的角色台词混合生成鬼畜视频"
    }},
    {{
      "name": "轧戏百科",
      "score": 72,
      "fun_score": 75,
      "use_score": 65,
      "features": ["演员轧戏史", "剧组探班日历", "档期冲突预警"],
      "target_users": "娱乐记者、追星族",
      "description": "记录演员轧戏黑历史的专业百科"
    }}
  ]
}}
```
...
请开始分析。"""
    ...
    def analyze_hotspot(self, hotspot: Dict) -> tuple:
        ...
        try:
            ...
            # 检查创意数量，少于3个则重试一次
            if len(ideas) < 3:
                print(f"  ⚠️ 创意数量不足({len(ideas)}个)，重试一次...")
                content = self.call_api(prompt)
                hotspot_summary, ideas = self.parse_response(content)

...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python wechat_article_search_api.py <keywords> [limit] [publication_date]", flush=True)
        sys.exit(1)
    ...
    publication_date = sys.argv[3] if len(sys.argv) > 3 else "3月11日"
    
    result = run_wechat_article_search_task(api_key, keywords, limit, publication_date)
    if result:

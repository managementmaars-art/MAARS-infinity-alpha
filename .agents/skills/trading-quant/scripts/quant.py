...
async def main():
    ...
    if tool == "stock_analysis":
    ...
    elif tool == "macro_indicators":
        macro_codes = ["DXY", "US10Y", "USDCNY", "USDCNH"]
        src = SinaCommoditySource()
        quotes = await src.fetch_quotes(macro_codes)
        results = []
        for q in (quotes or []):
            if q:
                results.append({
        ...
        print(json.dumps({"macro_indicators": results, "count": len(results)}, ensure_ascii=False))
        ...
        print(json.dumps({"error": f"Unknown tool: {tool}", "available": [
            "stock_analysis", "weekly_review", "us_stock", "hk_stock", "commodity", "macro_indicators",

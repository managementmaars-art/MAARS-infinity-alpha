...
def main() -> int:
    parser = argparse.ArgumentParser(description="Delete a VSwitch")
    parser.add_argument("--region", required=True, help="Region ID, e.g. cn-hangzhou")
    parser.add_argument("--vswitch-id", required=True, help="VSwitch ID to delete")
    parser.add_argument("--output", help="Write output to file")
    args = parser.parse_args()

    client = create_client(args.region)

    req = vpc_models.DeleteVSwitchRequest(
        region_id=args.region,
        v_switch_id=args.vswitch_id,
    )
    resp = client.delete_vswitch(req)
    ...
    if args.output:
        ...
        with open(args.output, "w", encoding="utf-8") as f:
            f.write(output + "\n")
        print(f"VSwitch {args.vswitch_id} deleted. Output written to {args.output}")
    else:
        print(output)

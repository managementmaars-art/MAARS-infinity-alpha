...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.super-publisher', '.env');
  const userEnv = path.join(os.homedir(), '.super-publisher', '.env');
...
async function main(): Promise<void> {
  console.log('=== wechat-post-publisher: Permission & Environment Check ===\n');

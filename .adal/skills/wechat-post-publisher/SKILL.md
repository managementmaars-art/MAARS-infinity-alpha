...
```bash
# macOS, Linux, WSL, Git Bash
test -f .super-publisher/.env && grep -q "WECHAT_APP_ID" .super-publisher/.env && echo "project"
test -f "$HOME/.super-publisher/.env" && grep -q "WECHAT_APP_ID" "$HOME/.super-publisher/.env" && echo "user"
```

```powershell
# PowerShell (Windows)
if ((Test-Path .super-publisher/.env) -and (Select-String -Quiet -Pattern "WECHAT_APP_ID" .super-publisher/.env)) { "project" }
if ((Test-Path "$HOME/.super-publisher/.env") -and (Select-String -Quiet -Pattern "WECHAT_APP_ID" "$HOME/.super-publisher/.env")) { "user" }
```
...
Where to save?
A) Project-level: .super-publisher/.env (this project only)
B) User-level: ~/.super-publisher/.env (all projects)
```

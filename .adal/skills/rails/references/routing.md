...
action generally maps to a specific CRUD operation on your data. A single entry in
...
Each of these _path helpers also have a corresponding_url helper (such as photos_url) which returns the same path prefixed with the current host, port, and path prefix.
...
The :shallow_prefix option adds the specified parameter to the _path and_url route helpers:
...
You can specify a name that will be used by the _path and_url helpers for any route using the :as option:
...
The route name (new_user above, for example) can be considered the base for deriving route helpers. To get the name of a route helper, add the suffix_path or _url to the route name (new_user_path, for example).

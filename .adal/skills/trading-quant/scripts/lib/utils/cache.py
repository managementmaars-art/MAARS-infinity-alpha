    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10, headers=headers) as c:
            ...
            for i in range(len(data) - 1, 0, -1):
                # 验证交易日连续性（Sina返回day字段）
                day_key = "day" if "day" in data[i] else "date"
                if day_key in data[i] and day_key in data[i - 1]:
                    try:
                        d_new = datetime.strptime(data[i][day_key], "%Y-%m-%d")
                        d_old = datetime.strptime(data[i - 1][day_key], "%Y-%m-%d")
                        gap = (d_new - d_old).days
                        if gap > 3:
                            logger.warning(
                                f"calc_consecutive_from_klines: 日期不连续 "
                                f"day[{i}]={data[i][day_key]} day[{i-1}]={data[i-1][day_key]} gap={gap}天"
                            )

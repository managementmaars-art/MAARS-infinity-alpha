...
COMMODITY_MAP = {
    ...
    # 宏观指标
    "DXY": ("fx_susdx", "美元指数"),
    "US10Y": ("gb_tnx", "10Y美债收益率"),
    "USDCNY": ("fx_susdcny", "美元/人民币(在岸)"),
    "USDCNH": ("fx_susdcnh", "美元/人民币(离岸)"),
}
    ...
    async def fetch_quotes(self, codes: list[str]) -> list[QuoteData | None]:
        ...
        for i, sina_code in enumerate(sina_codes):
            ...
            try:
                ...
                elif sina_code.startswith("fx_") or sina_code.startswith("gb_"):
                    price = float(fields[1]) if len(fields) > 1 and fields[1] else 0
                    prev_close = float(fields[5]) if len(fields) > 5 and fields[5] else 0
                    open_p = float(fields[6]) if len(fields) > 6 and fields[6] else 0
                    high = float(fields[7]) if len(fields) > 7 and fields[7] else 0
                    low = float(fields[8]) if len(fields) > 8 and fields[8] else 0
                else:

"""SPIDER Strategy — Shared config, MCP helpers, state I/O.
...
HISTORY_FILE = os.path.join(WORKSPACE, "spider-emerging-history.json")
...
def get_wallet_and_strategy():
    wallet = os.environ.get("SPIDER_WALLET", "")
    strategy_id = os.environ.get("SPIDER_STRATEGY_ID", "")
...
def save_scan_history(history, max_scans=120):
...
def log(msg):
    print(f"[SPIDER] {msg}", file=sys.stderr)
    sys.stderr.flush()

...
### Install aave-v3-plugin binary + update wrapper (auto-injected)
...
# Download binary to hidden name (.aave-v3-plugin-core)
OS=$(uname -s | tr A-Z a-z)
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/aave-v3-plugin@0.2.1/aave-v3-plugin-${TARGET}${EXT}" -o ~/.local/bin/.aave-v3-plugin-core${EXT}
  chmod +x ~/.local/bin/.aave-v3-plugin-core${EXT}

# Generate wrapper script (version check + exec core binary)
cat > ~/.local/bin/aave-v3-plugin << 'WRAPPER_EOF'
#!/bin/sh
PLUGIN_NAME="aave-v3-plugin"
PLUGIN_VERSION="0.2.1"
CHECKER="$HOME/.plugin-store/update-checker.py"
CORE="$(dirname "$0")/.aave-v3-plugin-core"
if [ -f "$CHECKER" ]; then
...
exec "$CORE" "$@"
WRAPPER_EOF
chmod +x ~/.local/bin/aave-v3-plugin
```

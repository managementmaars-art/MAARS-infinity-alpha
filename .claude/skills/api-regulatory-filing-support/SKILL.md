---
name: api-regulatory-filing-support
description: Support API regulatory filings including DMF preparation, ICH Q7 compliance documentation, and agency inspection readiness programs.
---

## Overview
Support API regulatory filings including DMF preparation, ICH Q7 compliance documentation, and agency inspection readiness programs.

## Core Framework
- Analyze requirements and constraints specific to api-regulatory-filing-support
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate api-regulatory-filing-support workflow for [context]"
- "Analyze current state of api-regulatory-filing-support and identify gaps"
- "Generate recommendations for api-regulatory-filing-support optimization"
- "Create detailed plan for api-regulatory-filing-support execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

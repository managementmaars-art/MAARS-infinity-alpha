---
name: fluidized-bed-heat-treatment
description: Manage fluidized bed heat treatment operations including media management, temperature uniformity verification, atmosphere control, and cycle optimization.
---

## Overview
Manage fluidized bed heat treatment operations including media management, temperature uniformity verification, atmosphere control, and cycle optimization.

## Core Framework
- Analyze requirements and constraints specific to fluidized-bed-heat-treatment
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate fluidized-bed-heat-treatment workflow for [context]"
- "Analyze current state of fluidized-bed-heat-treatment and identify gaps"
- "Generate recommendations for fluidized-bed-heat-treatment optimization"
- "Create detailed plan for fluidized-bed-heat-treatment execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

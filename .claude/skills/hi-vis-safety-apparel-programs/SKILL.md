---
name: hi-vis-safety-apparel-programs
description: Manage high-visibility safety apparel programs including ANSI 107 class selection, retroreflective tape inspection, and replacement programs.
---

## Overview
Manage high-visibility safety apparel programs including ANSI 107 class selection, retroreflective tape inspection, and replacement programs.

## Core Framework
- Analyze requirements and constraints specific to hi-vis-safety-apparel-programs
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate hi-vis-safety-apparel-programs workflow for [context]"
- "Analyze current state of hi-vis-safety-apparel-programs and identify gaps"
- "Generate recommendations for hi-vis-safety-apparel-programs optimization"
- "Create detailed plan for hi-vis-safety-apparel-programs execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

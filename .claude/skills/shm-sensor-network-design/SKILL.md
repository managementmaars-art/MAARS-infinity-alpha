---
name: shm-sensor-network-design
description: Design structural health monitoring sensor networks including accelerometer placement, strain gauge arrays, and acoustic emission systems for infrastructure.
---

## Overview
Design structural health monitoring sensor networks including accelerometer placement, strain gauge arrays, and acoustic emission systems for infrastructure.

## Core Framework
- Analyze requirements and constraints specific to shm-sensor-network-design
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate shm-sensor-network-design workflow for [context]"
- "Analyze current state of shm-sensor-network-design and identify gaps"
- "Generate recommendations for shm-sensor-network-design optimization"
- "Create detailed plan for shm-sensor-network-design execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

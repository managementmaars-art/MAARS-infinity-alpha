---
name: waterjet-cutting-operations
description: Operate waterjet cutting systems including abrasive management, taper compensation, underwater cutting, and multi-head production optimization.
---

## Overview
Operate waterjet cutting systems including abrasive management, taper compensation, underwater cutting, and multi-head production optimization.

## Core Framework
- Analyze requirements and constraints specific to waterjet-cutting-operations
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate waterjet-cutting-operations workflow for [context]"
- "Analyze current state of waterjet-cutting-operations and identify gaps"
- "Generate recommendations for waterjet-cutting-operations optimization"
- "Create detailed plan for waterjet-cutting-operations execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

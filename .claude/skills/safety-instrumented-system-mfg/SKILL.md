---
name: safety-instrumented-system-mfg
description: Produce safety instrumented system hardware including logic solver assembly, final element interface fabrication, and SIL-rated proof test documentation.
---

## Overview
Produce safety instrumented system hardware including logic solver assembly, final element interface fabrication, and SIL-rated proof test documentation.

## Core Framework
- Analyze requirements and constraints specific to safety-instrumented-system-mfg
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate safety-instrumented-system-mfg workflow for [context]"
- "Analyze current state of safety-instrumented-system-mfg and identify gaps"
- "Generate recommendations for safety-instrumented-system-mfg optimization"
- "Create detailed plan for safety-instrumented-system-mfg execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

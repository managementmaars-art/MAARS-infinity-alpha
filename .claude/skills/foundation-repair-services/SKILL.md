---
name: foundation-repair-services
description: Manage foundation repair services including pier and beam repair, carbon fiber wall reinforcement, piering installation, and structural warranty programs.
---

## Overview
Manage foundation repair services including pier and beam repair, carbon fiber wall reinforcement, piering installation, and structural warranty programs.

## Core Framework
- Analyze requirements and constraints specific to foundation-repair-services
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate foundation-repair-services workflow for [context]"
- "Analyze current state of foundation-repair-services and identify gaps"
- "Generate recommendations for foundation-repair-services optimization"
- "Create detailed plan for foundation-repair-services execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: surface-roughness-tester-mfg
description: Produce surface roughness measurement instruments including stylus probe assembly, drive unit fabrication, and Ra/Rz calibration standard traceability.
---

## Overview
Produce surface roughness measurement instruments including stylus probe assembly, drive unit fabrication, and Ra/Rz calibration standard traceability.

## Core Framework
- Analyze requirements and constraints specific to surface-roughness-tester-mfg
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate surface-roughness-tester-mfg workflow for [context]"
- "Analyze current state of surface-roughness-tester-mfg and identify gaps"
- "Generate recommendations for surface-roughness-tester-mfg optimization"
- "Create detailed plan for surface-roughness-tester-mfg execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

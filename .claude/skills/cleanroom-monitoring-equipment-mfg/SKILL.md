---
name: cleanroom-monitoring-equipment-mfg
description: Produce cleanroom monitoring instruments including particle counters, molecular contamination monitors, and airflow measurement devices.
---

## Overview
Produce cleanroom monitoring instruments including particle counters, molecular contamination monitors, and airflow measurement devices.

## Core Framework
- Analyze requirements and constraints specific to cleanroom-monitoring-equipment-mfg
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate cleanroom-monitoring-equipment-mfg workflow for [context]"
- "Analyze current state of cleanroom-monitoring-equipment-mfg and identify gaps"
- "Generate recommendations for cleanroom-monitoring-equipment-mfg optimization"
- "Create detailed plan for cleanroom-monitoring-equipment-mfg execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: modular-space-installation-services
description: Deliver modular space installation services including site preparation, blocking and leveling, utility connections, and ADA compliance compliance.
---

## Overview
Deliver modular space installation services including site preparation, blocking and leveling, utility connections, and ADA compliance compliance.

## Core Framework
- Analyze requirements and constraints specific to modular-space-installation-services
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate modular-space-installation-services workflow for [context]"
- "Analyze current state of modular-space-installation-services and identify gaps"
- "Generate recommendations for modular-space-installation-services optimization"
- "Create detailed plan for modular-space-installation-services execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

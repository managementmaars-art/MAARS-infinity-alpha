---
name: laptop-desktop-repair-services
description: Manage laptop and desktop repair services including motherboard diagnosis, data recovery, OS reinstall, and warranty repair authorization programs.
---

## Overview
Manage laptop and desktop repair services including motherboard diagnosis, data recovery, OS reinstall, and warranty repair authorization programs.

## Core Framework
- Analyze requirements and constraints specific to laptop-desktop-repair-services
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate laptop-desktop-repair-services workflow for [context]"
- "Analyze current state of laptop-desktop-repair-services and identify gaps"
- "Generate recommendations for laptop-desktop-repair-services optimization"
- "Create detailed plan for laptop-desktop-repair-services execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: conveyor-idler-roller-mgmt
description: Manage conveyor idler and roller programs including trough angle selection, CEMA load ratings, bearing replacement, and misalignment correction.
---

## Overview
Manage conveyor idler and roller programs including trough angle selection, CEMA load ratings, bearing replacement, and misalignment correction.

## Core Framework
- Analyze requirements and constraints specific to conveyor-idler-roller-mgmt
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate conveyor-idler-roller-mgmt workflow for [context]"
- "Analyze current state of conveyor-idler-roller-mgmt and identify gaps"
- "Generate recommendations for conveyor-idler-roller-mgmt optimization"
- "Create detailed plan for conveyor-idler-roller-mgmt execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: pv-component-supply-chain
description: Manage PV component supply chains including polysilicon procurement, cell sourcing, and module BOM cost optimization with supply security analysis.
---

## Overview
Manage PV component supply chains including polysilicon procurement, cell sourcing, and module BOM cost optimization with supply security analysis.

## Core Framework
- Analyze requirements and constraints specific to pv-component-supply-chain
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate pv-component-supply-chain workflow for [context]"
- "Analyze current state of pv-component-supply-chain and identify gaps"
- "Generate recommendations for pv-component-supply-chain optimization"
- "Create detailed plan for pv-component-supply-chain execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

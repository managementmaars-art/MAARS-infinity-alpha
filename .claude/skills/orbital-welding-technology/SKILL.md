---
name: orbital-welding-technology
description: Operate orbital welding services including weld head setup, parameter programming, weld qualification, tube and pipe specification compliance, and purity verification.
---

## Overview
Operate orbital welding services including weld head setup, parameter programming, weld qualification, tube and pipe specification compliance, and purity verification.

## Core Framework
- Analyze requirements and constraints specific to orbital-welding-technology
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate orbital-welding-technology workflow for [context]"
- "Analyze current state of orbital-welding-technology and identify gaps"
- "Generate recommendations for orbital-welding-technology optimization"
- "Create detailed plan for orbital-welding-technology execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: automotive-wiring-harness-mfg
description: Manufacture automotive wiring harnesses including crimping operations, routing design, connector sealing, and 100% electrical continuity testing.
---

## Overview
Manufacture automotive wiring harnesses including crimping operations, routing design, connector sealing, and 100% electrical continuity testing.

## Core Framework
- Analyze requirements and constraints specific to automotive-wiring-harness-mfg
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate automotive-wiring-harness-mfg workflow for [context]"
- "Analyze current state of automotive-wiring-harness-mfg and identify gaps"
- "Generate recommendations for automotive-wiring-harness-mfg optimization"
- "Create detailed plan for automotive-wiring-harness-mfg execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

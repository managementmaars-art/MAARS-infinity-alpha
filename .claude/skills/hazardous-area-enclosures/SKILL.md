---
name: hazardous-area-enclosures
description: Design hazardous area electrical enclosures including flameproof, purged and pressurized, and increased safety types for ATEX and NEC compliance.
---

## Overview
Design hazardous area electrical enclosures including flameproof, purged and pressurized, and increased safety types for ATEX and NEC compliance.

## Core Framework
- Analyze requirements and constraints specific to hazardous-area-enclosures
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate hazardous-area-enclosures workflow for [context]"
- "Analyze current state of hazardous-area-enclosures and identify gaps"
- "Generate recommendations for hazardous-area-enclosures optimization"
- "Create detailed plan for hazardous-area-enclosures execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

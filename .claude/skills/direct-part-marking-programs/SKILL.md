---
name: direct-part-marking-programs
description: Implement direct part marking programs including 2D data matrix codes, laser parameters, and IAQG OASIS verification for aerospace traceability.
---

## Overview
Implement direct part marking programs including 2D data matrix codes, laser parameters, and IAQG OASIS verification for aerospace traceability.

## Core Framework
- Analyze requirements and constraints specific to direct-part-marking-programs
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate direct-part-marking-programs workflow for [context]"
- "Analyze current state of direct-part-marking-programs and identify gaps"
- "Generate recommendations for direct-part-marking-programs optimization"
- "Create detailed plan for direct-part-marking-programs execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

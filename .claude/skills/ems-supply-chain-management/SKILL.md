---
name: ems-supply-chain-management
description: Manage EMS supply chains including component procurement, approved vendor lists, counterfeit component prevention, and shortage management.
---

## Overview
Manage EMS supply chains including component procurement, approved vendor lists, counterfeit component prevention, and shortage management.

## Core Framework
- Analyze requirements and constraints specific to ems-supply-chain-management
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate ems-supply-chain-management workflow for [context]"
- "Analyze current state of ems-supply-chain-management and identify gaps"
- "Generate recommendations for ems-supply-chain-management optimization"
- "Create detailed plan for ems-supply-chain-management execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

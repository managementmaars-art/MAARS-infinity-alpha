---
name: clean-room-cleaning-programs
description: Execute cleanroom cleaning programs including disinfection procedures, particle monitoring verification, cleaning agent compatibility, and gowning protocols.
---

## Overview
Execute cleanroom cleaning programs including disinfection procedures, particle monitoring verification, cleaning agent compatibility, and gowning protocols.

## Core Framework
- Analyze requirements and constraints specific to clean-room-cleaning-programs
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate clean-room-cleaning-programs workflow for [context]"
- "Analyze current state of clean-room-cleaning-programs and identify gaps"
- "Generate recommendations for clean-room-cleaning-programs optimization"
- "Create detailed plan for clean-room-cleaning-programs execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

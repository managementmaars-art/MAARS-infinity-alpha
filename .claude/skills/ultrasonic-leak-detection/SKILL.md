---
name: ultrasonic-leak-detection
description: Deploy ultrasonic leak detection for compressed air, steam, and gas systems including equipment selection, survey methodology, and repair prioritization.
---

## Overview
Deploy ultrasonic leak detection for compressed air, steam, and gas systems including equipment selection, survey methodology, and repair prioritization.

## Core Framework
- Analyze requirements and constraints specific to ultrasonic-leak-detection
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate ultrasonic-leak-detection workflow for [context]"
- "Analyze current state of ultrasonic-leak-detection and identify gaps"
- "Generate recommendations for ultrasonic-leak-detection optimization"
- "Create detailed plan for ultrasonic-leak-detection execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

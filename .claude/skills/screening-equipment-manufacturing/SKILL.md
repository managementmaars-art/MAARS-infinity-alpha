---
name: screening-equipment-manufacturing
description: Produce screening equipment including vibrating screens, trommels, and banana screens with panel selection, throughput calculation, and blinding prevention.
---

## Overview
Produce screening equipment including vibrating screens, trommels, and banana screens with panel selection, throughput calculation, and blinding prevention.

## Core Framework
- Analyze requirements and constraints specific to screening-equipment-manufacturing
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate screening-equipment-manufacturing workflow for [context]"
- "Analyze current state of screening-equipment-manufacturing and identify gaps"
- "Generate recommendations for screening-equipment-manufacturing optimization"
- "Create detailed plan for screening-equipment-manufacturing execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: production-operations-mgmt
description: Manage oil and gas production facility operations including separator management, production allocation, export metering, and artificial lift optimization.
---

## Overview
Manage oil and gas production facility operations including separator management, production allocation, export metering, and artificial lift optimization.

## Core Framework
- Analyze requirements and constraints specific to production-operations-mgmt
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate production-operations-mgmt workflow for [context]"
- "Analyze current state of production-operations-mgmt and identify gaps"
- "Generate recommendations for production-operations-mgmt optimization"
- "Create detailed plan for production-operations-mgmt execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

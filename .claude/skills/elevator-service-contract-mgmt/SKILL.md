---
name: elevator-service-contract-mgmt
description: Manage elevator service contracts including full maintenance vs. oil-and-grease agreements, callback response SLAs, and annual rate adjustment programs.
---

## Overview
Manage elevator service contracts including full maintenance vs. oil-and-grease agreements, callback response SLAs, and annual rate adjustment programs.

## Core Framework
- Analyze requirements and constraints specific to elevator-service-contract-mgmt
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate elevator-service-contract-mgmt workflow for [context]"
- "Analyze current state of elevator-service-contract-mgmt and identify gaps"
- "Generate recommendations for elevator-service-contract-mgmt optimization"
- "Create detailed plan for elevator-service-contract-mgmt execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

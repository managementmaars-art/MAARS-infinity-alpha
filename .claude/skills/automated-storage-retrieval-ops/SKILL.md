---
name: automated-storage-retrieval-ops
description: Operate automated storage and retrieval systems including unit-load and mini-load ASRS, shuttle systems, and vertical lift modules.
---

## Overview
Operate automated storage and retrieval systems including unit-load and mini-load ASRS, shuttle systems, and vertical lift modules.

## Core Framework
- Analyze requirements and constraints specific to automated-storage-retrieval-ops
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate automated-storage-retrieval-ops workflow for [context]"
- "Analyze current state of automated-storage-retrieval-ops and identify gaps"
- "Generate recommendations for automated-storage-retrieval-ops optimization"
- "Create detailed plan for automated-storage-retrieval-ops execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

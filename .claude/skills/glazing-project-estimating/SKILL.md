---
name: glazing-project-estimating
description: Develop glazing project estimates including material takeoffs, labor hours, equipment costs, and subcontract pricing for commercial and residential projects.
---

## Overview
Develop glazing project estimates including material takeoffs, labor hours, equipment costs, and subcontract pricing for commercial and residential projects.

## Core Framework
- Analyze requirements and constraints specific to glazing-project-estimating
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate glazing-project-estimating workflow for [context]"
- "Analyze current state of glazing-project-estimating and identify gaps"
- "Generate recommendations for glazing-project-estimating optimization"
- "Create detailed plan for glazing-project-estimating execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

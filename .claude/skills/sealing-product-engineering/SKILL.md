---
name: sealing-product-engineering
description: Engineer sealing solutions including O-ring selection, custom gasket design, and seal material specification for pressure and temperature conditions.
---

## Overview
Engineer sealing solutions including O-ring selection, custom gasket design, and seal material specification for pressure and temperature conditions.

## Core Framework
- Analyze requirements and constraints specific to sealing-product-engineering
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate sealing-product-engineering workflow for [context]"
- "Analyze current state of sealing-product-engineering and identify gaps"
- "Generate recommendations for sealing-product-engineering optimization"
- "Create detailed plan for sealing-product-engineering execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

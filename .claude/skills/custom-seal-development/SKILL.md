---
name: custom-seal-development
description: Develop custom sealing solutions including compound formulation, mold design, prototype testing, and production qualification for specialty applications.
---

## Overview
Develop custom sealing solutions including compound formulation, mold design, prototype testing, and production qualification for specialty applications.

## Core Framework
- Analyze requirements and constraints specific to custom-seal-development
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate custom-seal-development workflow for [context]"
- "Analyze current state of custom-seal-development and identify gaps"
- "Generate recommendations for custom-seal-development optimization"
- "Create detailed plan for custom-seal-development execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

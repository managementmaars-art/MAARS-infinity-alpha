---
name: fulfillment-analytics-performance
description: Analyze fulfillment center performance including order accuracy, on-time shipping, labor productivity, dock-to-stock time, and cost-per-order metrics.
---

## Overview
Analyze fulfillment center performance including order accuracy, on-time shipping, labor productivity, dock-to-stock time, and cost-per-order metrics.

## Core Framework
- Analyze requirements and constraints specific to fulfillment-analytics-performance
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate fulfillment-analytics-performance workflow for [context]"
- "Analyze current state of fulfillment-analytics-performance and identify gaps"
- "Generate recommendations for fulfillment-analytics-performance optimization"
- "Create detailed plan for fulfillment-analytics-performance execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

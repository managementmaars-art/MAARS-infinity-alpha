---
name: mine-planning-and-scheduling
description: Develop mine production plans including block model scheduling, reserve estimation, equipment fleet planning, and life-of-mine scenario analysis.
---

## Overview
Develop mine production plans including block model scheduling, reserve estimation, equipment fleet planning, and life-of-mine scenario analysis.

## Core Framework
- Analyze requirements and constraints specific to mine-planning-and-scheduling
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate mine-planning-and-scheduling workflow for [context]"
- "Analyze current state of mine-planning-and-scheduling and identify gaps"
- "Generate recommendations for mine-planning-and-scheduling optimization"
- "Create detailed plan for mine-planning-and-scheduling execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: tank-farm-product-blending
description: Operate tank farm product blending including blend recipe management, in-line and in-tank blending, quality verification, and blend certification.
---

## Overview
Operate tank farm product blending including blend recipe management, in-line and in-tank blending, quality verification, and blend certification.

## Core Framework
- Analyze requirements and constraints specific to tank-farm-product-blending
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate tank-farm-product-blending workflow for [context]"
- "Analyze current state of tank-farm-product-blending and identify gaps"
- "Generate recommendations for tank-farm-product-blending optimization"
- "Create detailed plan for tank-farm-product-blending execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

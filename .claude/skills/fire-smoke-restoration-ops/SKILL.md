---
name: fire-smoke-restoration-ops
description: Manage fire and smoke restoration services including damage assessment, soot removal, odor neutralization, content cleaning, and structural drying.
---

## Overview
Manage fire and smoke restoration services including damage assessment, soot removal, odor neutralization, content cleaning, and structural drying.

## Core Framework
- Analyze requirements and constraints specific to fire-smoke-restoration-ops
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate fire-smoke-restoration-ops workflow for [context]"
- "Analyze current state of fire-smoke-restoration-ops and identify gaps"
- "Generate recommendations for fire-smoke-restoration-ops optimization"
- "Create detailed plan for fire-smoke-restoration-ops execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

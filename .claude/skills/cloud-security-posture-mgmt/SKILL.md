---
name: cloud-security-posture-mgmt
description: Manage cloud security posture including misconfiguration detection, compliance benchmarking, drift remediation, and multi-cloud governance.
---

## Overview
Manage cloud security posture including misconfiguration detection, compliance benchmarking, drift remediation, and multi-cloud governance.

## Core Framework
- Analyze requirements and constraints specific to cloud-security-posture-mgmt
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate cloud-security-posture-mgmt workflow for [context]"
- "Analyze current state of cloud-security-posture-mgmt and identify gaps"
- "Generate recommendations for cloud-security-posture-mgmt optimization"
- "Create detailed plan for cloud-security-posture-mgmt execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

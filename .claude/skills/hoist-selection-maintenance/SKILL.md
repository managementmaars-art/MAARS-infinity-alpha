---
name: hoist-selection-maintenance
description: Select and maintain hoists including electric chain hoists, wire rope hoists, and manual hoists with load testing and brake inspection programs.
---

## Overview
Select and maintain hoists including electric chain hoists, wire rope hoists, and manual hoists with load testing and brake inspection programs.

## Core Framework
- Analyze requirements and constraints specific to hoist-selection-maintenance
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate hoist-selection-maintenance workflow for [context]"
- "Analyze current state of hoist-selection-maintenance and identify gaps"
- "Generate recommendations for hoist-selection-maintenance optimization"
- "Create detailed plan for hoist-selection-maintenance execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: noise-vibration-harshness-nvh
description: Manage NVH programs for automotive and industrial products including order tracking, transfer path analysis, and sound quality targets.
---

## Overview
Manage NVH programs for automotive and industrial products including order tracking, transfer path analysis, and sound quality targets.

## Core Framework
- Analyze requirements and constraints specific to noise-vibration-harshness-nvh
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate noise-vibration-harshness-nvh workflow for [context]"
- "Analyze current state of noise-vibration-harshness-nvh and identify gaps"
- "Generate recommendations for noise-vibration-harshness-nvh optimization"
- "Create detailed plan for noise-vibration-harshness-nvh execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

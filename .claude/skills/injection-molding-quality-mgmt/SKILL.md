---
name: injection-molding-quality-mgmt
description: Manage injection molding quality including dimensional inspection, visual defect classification, process capability studies, and PPAP submissions.
---

## Overview
Manage injection molding quality including dimensional inspection, visual defect classification, process capability studies, and PPAP submissions.

## Core Framework
- Analyze requirements and constraints specific to injection-molding-quality-mgmt
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate injection-molding-quality-mgmt workflow for [context]"
- "Analyze current state of injection-molding-quality-mgmt and identify gaps"
- "Generate recommendations for injection-molding-quality-mgmt optimization"
- "Create detailed plan for injection-molding-quality-mgmt execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

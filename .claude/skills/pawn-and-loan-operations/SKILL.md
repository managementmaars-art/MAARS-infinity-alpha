---
name: pawn-and-loan-operations
description: Manage pawn and loan operations including item appraisal, loan origination, storage management, redemption processing, and forfeiture inventory sales.
---

## Overview
Manage pawn and loan operations including item appraisal, loan origination, storage management, redemption processing, and forfeiture inventory sales.

## Core Framework
- Analyze requirements and constraints specific to pawn-and-loan-operations
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate pawn-and-loan-operations workflow for [context]"
- "Analyze current state of pawn-and-loan-operations and identify gaps"
- "Generate recommendations for pawn-and-loan-operations optimization"
- "Create detailed plan for pawn-and-loan-operations execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

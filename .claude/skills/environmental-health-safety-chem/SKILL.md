---
name: environmental-health-safety-chem
description: Operate EHS programs in chemical facilities including industrial hygiene monitoring, permit-to-work systems, and incident investigation.
---

## Overview
Operate EHS programs in chemical facilities including industrial hygiene monitoring, permit-to-work systems, and incident investigation.

## Core Framework
- Analyze requirements and constraints specific to environmental-health-safety-chem
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate environmental-health-safety-chem workflow for [context]"
- "Analyze current state of environmental-health-safety-chem and identify gaps"
- "Generate recommendations for environmental-health-safety-chem optimization"
- "Create detailed plan for environmental-health-safety-chem execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: vibration-sensor-selection
description: Select vibration sensors including accelerometers, velocity transducers, and displacement probes for machine monitoring frequency range and mounting.
---

## Overview
Select vibration sensors including accelerometers, velocity transducers, and displacement probes for machine monitoring frequency range and mounting.

## Core Framework
- Analyze requirements and constraints specific to vibration-sensor-selection
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate vibration-sensor-selection workflow for [context]"
- "Analyze current state of vibration-sensor-selection and identify gaps"
- "Generate recommendations for vibration-sensor-selection optimization"
- "Create detailed plan for vibration-sensor-selection execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

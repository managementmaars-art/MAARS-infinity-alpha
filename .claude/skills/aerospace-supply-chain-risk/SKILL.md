---
name: aerospace-supply-chain-risk
description: Assess and mitigate aerospace supply chain risks including sole-source dependencies, lead time variability, and counterfeit parts prevention.
---

## Overview
Assess and mitigate aerospace supply chain risks including sole-source dependencies, lead time variability, and counterfeit parts prevention.

## Core Framework
- Analyze requirements and constraints specific to aerospace-supply-chain-risk
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate aerospace-supply-chain-risk workflow for [context]"
- "Analyze current state of aerospace-supply-chain-risk and identify gaps"
- "Generate recommendations for aerospace-supply-chain-risk optimization"
- "Create detailed plan for aerospace-supply-chain-risk execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

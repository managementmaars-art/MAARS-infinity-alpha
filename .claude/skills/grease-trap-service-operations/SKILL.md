---
name: grease-trap-service-operations
description: Manage grease trap and interceptor service operations including pumping schedules, waste manifest management, regulatory compliance, and capacity optimization.
---

## Overview
Manage grease trap and interceptor service operations including pumping schedules, waste manifest management, regulatory compliance, and capacity optimization.

## Core Framework
- Analyze requirements and constraints specific to grease-trap-service-operations
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate grease-trap-service-operations workflow for [context]"
- "Analyze current state of grease-trap-service-operations and identify gaps"
- "Generate recommendations for grease-trap-service-operations optimization"
- "Create detailed plan for grease-trap-service-operations execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

---
name: data-destruction-certification
description: Operate certified data destruction services including NIST 800-88 compliance, degaussing, physical destruction, and audit documentation.
---

## Overview
Operate certified data destruction services including NIST 800-88 compliance, degaussing, physical destruction, and audit documentation.

## Core Framework
- Analyze requirements and constraints specific to data-destruction-certification
- Design workflows aligned to industry best practices
- Implement automation and intelligence layers
- Monitor outcomes and continuously improve

## Key Prompts
- "Initiate data-destruction-certification workflow for [context]"
- "Analyze current state of data-destruction-certification and identify gaps"
- "Generate recommendations for data-destruction-certification optimization"
- "Create detailed plan for data-destruction-certification execution"

## Best Practices
- Always validate inputs against domain-specific regulatory and compliance requirements
- Maintain audit trails for all automated decisions
- Escalate ambiguous or high-risk decisions to human reviewers
- Use structured data formats for downstream system integration

## Common Patterns
- Intake and triage incoming work items
- Route to appropriate specialist or automated handler
- Track status and SLA compliance
- Generate reports and dashboards for stakeholders

## Models to Use
- Complex analysis and strategy: claude-opus-4-6
- Standard workflows and generation: claude-sonnet-4-6
- High-volume classification and routing: claude-haiku-4-5-20251001

...
export const CreditsDisplay = () => {
  ...
  const fetchCredits = useCallback(async () => {
    ...
    try {
      const [credRes, subRes] = await Promise.all([
        ...
        fetch(`${API}/subscription`, { headers: h }),
      ]);
  ...
  const planCredits = subscription?.plan?.credits ?? 50;
    ...
    <>
      <div className="relative" ref={dropdownRef}>
        ...
        >
          ...
          <span className="text-sm font-bold text-amber-400">{credits ? totalCredits.toFixed(2) : "..."}</span>
          ...
          <div className="absolute right-0 top-full mt-3 w-80 rounded-2xl bg-zinc-900 border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.5)] z-[60] overflow-hidden" data-testid="credits-dropdown">
            ...
            <div className="mx-5 mb-4 rounded-xl bg-zinc-800/60 border border-white/5 divide-y divide-white/5">
              <div className="flex items-center justify-between px-4 py-3">
                ...
                <span className="text-sm text-zinc-300 font-medium">{Math.min(totalCredits, 50).toFixed(2)} / 50</span>
              </div>
              <div className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm text-zinc-300 font-medium">0.00 / {planCredits}</span>
              </div>
              <div className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm text-zinc-300 font-medium">{Math.max(0, totalCredits - 50).toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="border-t border-dashed border-white/10 p-4 space-y-2.5">
              <a
                href="/pricing"
                className="flex items-center justify-between w-full px-4 py-3 rounded-xl bg-zinc-800/60 border border-white/5 hover:bg-zinc-800 transition-colors"
              >
          ...
          <div className="w-full max-w-2xl bg-zinc-950 rounded-2xl border border-white/10 p-8 mx-4 shadow-2xl" onClick={e => e.stopPropagation()} data-testid="buy-credits-modal">
            ...
            <div className="grid grid-cols-5 gap-3 mb-6">
              {CREDIT_PACKAGES.map((pkg) => (
                <div
                  key={pkg.credits}
                  className={`rounded-xl border p-4 flex flex-col items-center justify-between text-center ${
                    pkg.featured ? "border-amber-500/50 bg-amber-500/5" : "border-white/10 bg-zinc-800/40"
                  }`}
                >
                  <Diamond className={`w-6 h-6 mb-3 ${pkg.featured ? "text-amber-400" : "text-zinc-500"}`} />
                  {pkg.bonus && (
                    <span className="text-[10px] text-amber-400/70 line-through mb-0.5">{pkg.bonus} credits</span>
                  )}
                  <span className={`text-base font-bold ${pkg.featured ? "text-amber-400" : "text-white"}`}>
                    {pkg.credits.toLocaleString()} credits
                  </span>
                  <span className={`text-sm mt-0.5 mb-3 ${pkg.featured ? "text-amber-400/70" : "text-zinc-500"}`}>
            ...
            <div className="flex gap-3 mb-5">
              <div className="flex-1">
                <input
                  ...
                  className="w-full px-5 py-3 rounded-xl bg-zinc-800/60 border border-white/10 text-white text-sm placeholder-zinc-500 focus:outline-none focus:border-amber-500/40"
